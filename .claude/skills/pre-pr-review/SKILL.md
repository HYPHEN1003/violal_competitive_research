---
name: pre-pr-review
description: Use when user says "PRレビュー", "PR前チェック", "レビューして", "マージ前確認", or is about to create a pull request and needs a multi-AI quality check with MCO.
---

# PR前レビュー — MCO 3AIレビュースキル

PR作成前に、3つのAI（Claude Code, Codex, Gemini）で同時にコードレビューを実行し、結果を保存する。
...
mco review --repo . --diff \
  ...
  --providers claude,codex,gemini --format report
```
...
## サマリー
[3つのAIの指摘をまとめた要約。多数決で確認された指摘はconfirmed、1つのAIだけの指摘はneeds-verificationとする]
...
- confirmed（2つ以上のAIが指摘）は対応を強く推奨する
